-- NEXUS Live Integration Database Schema
-- Complete schema for ML, predictions, architecture analysis, and live data
...
CREATE INDEX idx_predictions_tenant ON predictions_cache (tenant_id);
CREATE INDEX idx_predictions_type ON predictions_cache (prediction_type);
CREATE INDEX idx_predictions_created ON predictions_cache (created_at DESC);
...
CREATE INDEX idx_feedback_tenant ON ml_feedback (tenant_id);
CREATE INDEX idx_feedback_action ON ml_feedback (user_action);
CREATE INDEX idx_feedback_created ON ml_feedback (created_at DESC);
...
CREATE INDEX idx_principles_category ON construction_principles (category);
CREATE INDEX idx_principles_confidence ON construction_principles (confidence_score DESC);
...
CREATE INDEX idx_conflicts_tenant ON schedule_conflicts (tenant_id);
CREATE INDEX idx_conflicts_project ON schedule_conflicts (project_id);
...
CREATE INDEX idx_analysis_type ON architecture_analysis_reports (analysis_type);
CREATE INDEX idx_analysis_score ON architecture_analysis_reports (production_readiness_score DESC);
CREATE INDEX idx_analysis_created ON architecture_analysis_reports (created_at DESC);
...
CREATE INDEX idx_quality_path ON code_quality_metrics (file_path);
CREATE INDEX idx_quality_analyzed ON code_quality_metrics (analyzed_at DESC);
...
CREATE INDEX idx_perf_tenant ON performance_metrics (tenant_id);
...
CREATE INDEX idx_model_metrics_created ON ml_model_metrics (created_at DESC);
...
-- Insert sample architecture analysis report
...
-- Schedule periodic cleanup (requires pg_cron extension in production)
-- SELECT cron.schedule('cleanup-predictions', '0 * * * *', 'SELECT cleanup_expired_predictions();');
...
DO $$
BEGIN
    RAISE NOTICE '✅ NEXUS Live Integration Schema created successfully!';
