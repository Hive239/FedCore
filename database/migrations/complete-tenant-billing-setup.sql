-- ============================================
-- COMPLETE TENANT BILLING AND SETTINGS SETUP
-- Run this file to set up all tenant billing features
-- ============================================
...
-- Alternative function that accepts a user_id parameter
...
-- Add created_at to user_tenants if missing
...
CREATE INDEX IF NOT EXISTS idx_user_tenants_user_tenant ON public.user_tenants(user_id, tenant_id);
...
DROP POLICY IF EXISTS billing_history_tenant_write ON public.billing_history;
DROP POLICY IF EXISTS usage_tracking_tenant_isolation ON public.usage_tracking;
DROP POLICY IF EXISTS usage_tracking_tenant_write ON public.usage_tracking;
...
DROP TRIGGER IF EXISTS update_limits_on_subscription_change ON tenants;
...
GRANT EXECUTE ON FUNCTION public.track_usage(UUID, TEXT, DECIMAL) TO authenticated;
...
-- STEP 11: Ensure default tenants
-- ============================================
...
-- STEP 12: Initialize tenant settings for existing tenants
-- ============================================
...
DO $$
BEGIN
    RAISE NOTICE '✅ Tenant billing and settings setup completed successfully!';
    RAISE NOTICE '';
    RAISE NOTICE '📊 Subscription Tiers Configured:';
    RAISE NOTICE '  • Starter: $29/user/month (10 users, 25 projects, 10GB)';
    RAISE NOTICE '  • Professional: $59/user/month (50 users, unlimited projects, 100GB)';
    RAISE NOTICE '  • Enterprise: $99/user/month (unlimited users/projects, 1TB)';
    RAISE NOTICE '';
    RAISE NOTICE '🔧 Functions Created:';
    RAISE NOTICE '  • get_user_tenant_id() - Gets current user tenant';
    RAISE NOTICE '  • user_has_tenant_access() - Checks tenant access';
    RAISE NOTICE '  • track_usage() - Tracks usage metrics';
    RAISE NOTICE '  • get_tenant_usage() - Gets current usage stats';
    RAISE NOTICE '';
    RAISE NOTICE '🔒 Security:';
    RAISE NOTICE '  • RLS enabled on all billing tables';
    RAISE NOTICE '  • Tenant isolation policies active';
    RAISE NOTICE '';
    RAISE NOTICE '✨ Next Steps:';
    RAISE NOTICE '  1. Configure Stripe webhook endpoints';
    RAISE NOTICE '  2. Set STRIPE_SECRET_KEY in environment';
    RAISE NOTICE '  3. Test subscription upgrades';
END $$;