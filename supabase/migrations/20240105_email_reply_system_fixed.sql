-- Email Reply System Schema (FIXED VERSION)
...
DROP TRIGGER IF EXISTS generate_conversation_thread_key ON conversations;
...
BEGIN
    ...
    SELECT 
        reply_to_email,
        inbound_domain
    INTO v_reply_to, v_inbound_domain
    FROM email_settings
    WHERE tenant_id = v_tenant_id
    AND is_active = TRUE;
    
    -- If we have an inbound domain, create unique reply-to
    IF v_inbound_domain IS NOT NULL AND v_thread_key IS NOT NULL THEN
        v_reply_to := v_thread_key || '@' || v_inbound_domain;
    ELSIF v_reply_to IS NULL THEN
        v_reply_to := 'noreply@projectpro.app';
    END IF;
    ...
    SET 
        ...
        email_references = v_last_email_id
...
GRANT EXECUTE ON FUNCTION generate_email_thread_key TO authenticated;
GRANT EXECUTE ON FUNCTION prepare_email_with_threading TO authenticated;