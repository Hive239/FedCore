-- Complete Tenant Management System for ProjectPro (FINAL VERSION)
-- This handles enum types and policy dependencies
...
DROP POLICY IF EXISTS "Users can manage their tenant's tasks" ON tasks;
DROP POLICY IF EXISTS "Users can view their tenant's events" ON schedule_events;
DROP POLICY IF EXISTS "Users can manage their tenant's events" ON schedule_events;
DROP POLICY IF EXISTS "Users can view their tenant's vendors" ON vendors;
DROP POLICY IF EXISTS "Users can manage vendors" ON vendors;

-- Step 2: Handle the role column properly
-- First, check if user_role enum exists and what values it has
DO $$ 
BEGIN
    -- Drop the enum type if it exists (we'll use varchar instead)
    DROP TYPE IF EXISTS user_role CASCADE;
END $$;

-- Now alter the role column to use varchar
...
-- Step 4: Create invitations table for team member invites
...
-- Step 6: Create or replace trigger for new user signups
...
-- Step 7: Create function to invite team members
...
-- Step 8: Enable RLS on tables (if not already enabled)
...
-- Step 10: Create RLS policies for user_tenants
...
-- Step 11: Create RLS policies for tenant_invitations
...
-- Step 12: Policies for other tables
-- Tasks
...
-- Vendors (simplified - vendors might not have tenant_id)
...
-- Step 13: Function to check user permissions
...
BEGIN
    ...
    -- Define permission matrix (without 'manager' role for now)
    CASE p_permission
        WHEN 'view' THEN
...
-- Step 14: Assign existing users to default tenant
DO $$
DECLARE
...
BEGIN
    ...
    ON CONFLICT (name) WHERE name = 'Default Organization' DO UPDATE SET name = tenants.name
...
-- Step 15: Add trade column to schedule_events if needed
ALTER TABLE schedule_events ADD COLUMN IF NOT EXISTS trade TEXT;

-- Step 16: Add tenant_id to vendors if needed
